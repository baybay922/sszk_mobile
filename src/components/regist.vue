<template>
  <div class="regist mt30">
    <!-- 注册第一步，选择注册类型 -->
    <div v-show="tab == '1'" class="talignc">
      <div class="regbox mt30">
        <p class="title">我是企业负责人</p>
        <div class="mt30">
          <ul class="uls">
            <li>开通企业账号</li>
            <li>管理下属销售人员</li>
          ...
          <img src="../images/enterprise.png" class="regimg">
        </div>
        <mt-button type="primary" @click="compnext('B')" class="resbtn">立即注册</mt-button>
      </div>
      <div class="regbox mt30">
        <p class="title">我是个体合伙人</p>
        <div class="mt30">
          <ul class="uls">
            <li>开通个人账号</li>
            <li>查看最新产品</li>
          ...
          <img src="../images/individual.png" class="regimg">
        </div>
        <mt-button type="primary" @click="compnext('C')" class="resbtn">立即注册</mt-button>
      </div>
    </div>
    <!-- 注册第二步，收填验证码  -->
    <div v-show="tab == '2'" class="tabb">
...
export default {
  ...
  data () {
    return {
      tab: '1',//步骤
      isregist: this.$route.query.isregist ? this.$route.query.isregist : '',
        ...
        if(len == 4 || len == 11){
          loginService.marketInfo(this.loginForm.identifier).then(r =>{
            if(r.flag == 20000){
              ...
              // this.loginForm.internalNumber = r.data.internalNumber;
              $(".market-info").show();
            }
          })
        }
    ...
    tonext() {
      ...
      if(!myreg.test(this.loginForm.loginName) || this.loginForm.loginName == "" || this.loginForm.loginName.length < 11){
        Toast("请输入正确的手机号")
          ...
          loginService.sendCode(this.loginForm.loginName, this.loginForm.imgValidateCode, this.loginForm.msgValidateCode, this.orgType, this.type, null,this.loginForm.identifier).then(r => {
        console.log(r)
        if(r.flag == 20000) {
          if(r.data == null){
            if(this.orgType == 'B') {//如果是企业注册，跳去regist2b页面完善企业信息，因为上传图片插件不支持vue判断语法
            ...
            if(r.data.flag == 30100){
              if(r.data.status == 1){
                ...
                if(r.data.orgType == 'B') {//如果是企业注册，跳去regist2b页面完善企业信息，因为上传图片插件不支持vue判断语法
                  let path = {path: '/regist2b', query: {}}
                  this.$router.push(path)
                }else if(r.data.orgType == 'C') {//如果是个人注册，跳去regist2c页面完善个人信息
                  let path = {path: '/regist2c', query: {}}
                  this.$router.push(path)
                }
              }
