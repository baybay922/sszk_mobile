<template>
  <div class="login common-margin-top">
    ...
    <div class="inputbox" v-if="logintab == '1'">
      ...
      <p class="othdo mt30">
        <a class="fl" href="#/regSelectType">立即注册</a>
     ...
     <div class="inputbox" v-if="logintab == '2'">
      ...
      <input type="password" v-model="loginForm.pwd" placeholder="请输入密码" class="mt30 info-box pwdBxo"> -->
      <mt-button type="primary" class="loginbutton mt30" @click="submit">登录</mt-button>
      <p class="othdo mt30"><a class="fl" href="#/regSelectType">立即注册</a>
...
export default {
  ...
  methods: {
    goForgetPwd(){
      let path = {path: '/getpwd', query: {orgType:"B"}}
    ...
    submit() {
      ...
      if(this.logintab == '2'){//密码登录
        loginService.login(this.loginForm.loginName, this.loginForm.pwd).then(r => {
          console.log(r)
          if (r.flag == 20000) {
            ...
            Toast(r.msg)
            let path = {path: '/regSelectType', query: {}}
                this.$router.push(path) 
            ...
            Toast(r.msg)
            let path = {path: '/regist', query: {isregist: 'false'}}
            this.$router.push(path)
            ...
            console.log(r)
            let rps = r.data.ystContract;
            let path = {path: '/homepage', query: {rps:rps}}
                this.$router.push(path)
          }else{
            Toast(r.msg)
          }
        ...
        loginService.login2(this.loginForm.loginName, this.loginForm.imgValidateCode, this.loginForm.msgValidateCode).then(r => {
          console.log(r)
          if (r.flag == 20000) {
            if(r.data.orgType == "P" || r.data.orgType == "D"){
              let path = {path: '/supplierOrder', query: {}}
